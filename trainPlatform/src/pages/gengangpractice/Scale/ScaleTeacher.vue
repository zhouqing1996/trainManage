  ...
  <div class="container">
    ...
    <div v-if="surveyList" v-for="(item,index) in surveyList" :key="index">
      <el-card class="box-card">
        ...
        <div class="text item">
          <router-link :to="{ path: '/requirement/surveyview',query:{ id: item.id,flag:20,status:item.status }}">
          ...
          <router-link :to="{ path: '/requirement/surveypublish', query: { id: item.id, flag: 20 }}" >
          ...
          <router-link :to="{ path: '/requirement/datastatistics', query: { id: item.id,flag:20 }}">
...
<script>
  ...
  export default {
    ...
    methods: {
      getdata:function(){
        let that=this;
        that.$http.post('/gengangpractice/evaluate/gettable2',{
          page:1,
          username:that.user
        }).then(function (res) {
      ...
      *editItem(item) {
        ...
        yield this.$router.push({path:'/requirement/surveyedit',query:{id: item.id,flag:20}});
      }
    },
