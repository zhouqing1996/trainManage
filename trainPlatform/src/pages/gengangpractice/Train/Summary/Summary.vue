<template>
  <div>
    <div v-if="listView==0">
      ...
      <template>
        <el-table :data="gengangpractice" border style="width: 100%">
          ...
          <el-table-column label="操作">
            <el-button slot-scope="scope" @click="summaryView(scope.$index)">查看实习总结</el-button>
          </el-table-column>
        </el-table>
      </template>
    </div>
          ...
          <div class="inforStyle">
            ...
            <div>
              ...
              <div class="distence">
                <span class="span">内容:</span>
                <span>
                      <el-input style="width: 60%" type="textarea" :disabled="power.userkind==2" :rows="4" placeholder="请输入内容" v-model="comEndEva">
                      </el-input>
                    </span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getdata: function () {
        let that = this
        this.$http.post('/gengangpractice/evaluate/getdata2', { recruitId: this.row.id }).then(function (res) {
          console.log(res.data.data)
          that.info = res.data.data
          that.stuInfo = that.info['stuInfo']
          that.gengangpractice = []
          for (var i = 0; i < that.stuInfo.length; i++) {
            that.gengangpractice[i] = {}
            that.gengangpractice[i].stuName = that.stuInfo[i][0]['stuName']
            that.gengangpractice[i].sno = that.stuInfo[i][0]['sno']
            that.gengangpractice[i].major = that.stuInfo[i][0]['major']
          }
        })
        this.$http.post('/gengangpractice/remind/getremind2', { recruitId: this.row.id }).then(function (res) {
          that.tableData = res.data.data
        })
      },
      summaryView: function (index) {
        this.summary = this.gengangpractice[index]
        ...
        let that = this
        that.$http.post('/gengangpractice/evaluate/endsummary', {
          stuId: that.stuId
        }).then((res) => {
      ...
      exportWd () {
        this.$http.post('/gengangpractice/export/sumword', { stu: this.summary, summary: this.summaryContent }).then(function (res) {
      ...
      handleDelete (id) {
        let that = this
        this.$http.post('/gengangpractice/remind/deletremind', { id: id }).then(function (res) {
          that.tableData = res.data.data
          console.log(res.data.data)
        })
      },
      ...
      handleCheckedDaysChange (value) {
        ...
        let that = this
        this.$http.post('/gengangpractice/remind/addremind2', { form: this.form, recruitId: this.row.id }).then(function (res) {
          that.tableData = res.data.data
          console.log(res.data.data)
        })
      },
      ensure2 () {
        ...
        let that = this
        this.$http.post('/gengangpractice/remind/editremind2', { form: this.form2, recruitId: this.row.id }).then(function (res) {
          that.tableData = res.data.data
          console.log(res.data.data)
        })
      }
