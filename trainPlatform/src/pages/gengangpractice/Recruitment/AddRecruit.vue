...
<script>
    export default {
      ...
      methods: {
        visible (param) {
          if (param == 1) {
            ...
            this.$http.post('/gengangpractice/recruitment/addrecruit', { form: this.form, major: this.mymajor, grade: this.mygrade, company: com }).then(function (res) {
                that.$emit('child', 1)
            })
      ...
      mounted () {
        let that = this
        this.$http.post('/gengangpractice/recruitment/company', {power: this.power}).then(function (res) {
