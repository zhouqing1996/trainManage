<template>
  <div>
    <el-dialog title="实习信息" :visible.sync="mapvisible" @close='closeDialog'>
      <el-form :model="form">
  ...
  export default {
    ...
    methods: {
      visible (param) {
        if (param == 1) {
          let that = this
          this.$http.post('/gengangpractice/recruitment/addmap', { rowId: this.rowId, form: this.form, lng: this.lng, lat: this.lat, address: this.address }).then(function (res) {
