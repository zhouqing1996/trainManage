<template>
  <div class="container">
    <el-button style="margin-left: 40px" type="warning" @click="$router.push({path:'/requirement/surveyindex',query:{id: 0}})">+ 新建问卷</el-button>
    <div v-for="(item,index) in surveyList" :key="index">
      <el-card class="box-card">
        ...
        <div class="text item">
            <router-link :to="{ path: '/requirement/surveyview', query: { id: item.id,status:item.status }}">
...
<!--          <el-button @click="iterator"></el-button>-->
        </div>
      </el-card>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getdata:function(){
        let that=this;
        that.$http.post('/requirement/require/getdata',{
          page:1,
          username:that.user
        }).then(function (res) {
      ...
      *publishItem(item,flag) {
        ...
        let that = this;
        this.$http.post('/requirement/require/publishsurvey',{
          id:item.id,
          status:state
        }).then(function (res) {
          console.log(res.data);
      ...
      *deleteItem(item) {
        ...
        let that = this;
        that.$http.post('/requirement/require/deletesurvey',{id:item.id}).then(function (res) {
          console.log(res.data);
      ...
      *editItem(item) {
        ...
        yield this.$router.push({path:'/requirement/surveyedit',query:{id: item.id}});
      },
