    ...
    <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
                ...
                <div v-for="(item, index2) in quData.questions" :key="index2" style="margin-top: 20px">
                  ...
                  <div style="margin-top: 10px" :span="24" v-if="item['type']=='radio'||item['type']=='checkbox'||item['type']=='rate'">
                    ...
                    <el-row>
                      <el-col :span="10">
                        <template>
                          <el-table :data="data[index2]" style="width: 100%" border size="mini">
                            <el-table-column prop="opt" label="选项" width="170"></el-table-column>
                            <el-table-column prop="sum" label="小计" width="50"></el-table-column>
                            <el-table-column prop="percent" label="比例" width="200">
                      ...
                      <el-col :span="12" style="margin-left: 5%;">
                        <data-chart style="width: 100%" :opt="item['options']" :rate="quData['answerInfo'][index2]"></data-chart>
                      </el-col>
                    </el-row>
                  </div>
      ...
      <el-tab-pane name="second">
        ...
        <div style="width: 80%;background-color: white;padding-top: 2%;margin-left:10%;padding-bottom:40px;">
          ...
          <el-button style="width:100px;height:30px;margin-right: 30px;margin-top: 20px;" type="warning"  v-on:click="download()" class="imptbtn">+ 数据下载</el-button>
    ...
    export default {
      ...
      methods: {
        ...
        download () {
          ...
          const filterVal = []
          const excelData = []
          for (var i = 0; i <= this.quData.questions.length; i++) {
            excelData[0] = {}
            if (tHeader.length==0){
              tHeader.push('序号')
            } else{
              tHeader.push(this.quData.questions[i-1]['topic'])
            }
            filterVal.push(i+'')
          }
          for (var i = 0; i < this.paperInfo.content.length; i++) {
            excelData[i] = {}
            for (var j = 0; j <= this.quData.questions.length; j++) {
              if (j == 0){
                excelData[i][j] = i+1;
              } else{
                excelData[i][j] = this.paperInfo.content[i][j-1]['content']
              }
            }
          }
          require.ensure([], () => {
            ...
            console.log(data)
            export_json_to_excel(tHeader, data, this.quData['surveyInfo']['title'])
          })
        },
