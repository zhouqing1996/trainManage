  ...
  <div class="container">
    <div class="qu-wrap">
      ...
      <footer>
        ...
        <div v-if="flag == 40" class="ctrl-part">
          <span @click="iterator = saveBtn(); iterator.next()">保存需求量表</span>
        </div>
        <div v-if="flag != 20&&flag != 40" class="ctrl-part">
    ...
    methods: {
      ...
      *saveBtn() {
        ...
        if (that.flag == 20){
          ...
          yield that.showPrompt(`确认要保存需求调查量表？`);
        }else{
          yield that.showPrompt(`确认要保存问卷？`);
        }
        if (that.quData.id == undefined){
          that.$http.post('/requirement/require/insertsurvey', {
            quData:that.quData,
            username:that.user
          }).then((res)=>{
            console.log(111)
            ...
            if(res.data.message==1){
              if (that.flag == 10){
                ...
                alert("量表添加成功！");
                that.$router.push({ path:'/practice/teacher',query:{table:1}});
              }else if(that.flag == 40){
                alert("需求调查添加成功！");
                that.$router.push({ path:'/apprenticeship/enroll/demandlist'});
              }else{
                alert("问卷添加成功！");
