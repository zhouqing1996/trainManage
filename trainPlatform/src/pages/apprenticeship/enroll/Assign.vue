<template>
  <div class="display1">
    <button class="btn1" v-on:click="major1" v-bind:class="{ active: isActiveOne }">招生指标</button>
    ...
    <div class="display2" v-show="major">
      <div class="searchmem">
        ...
        <el-dialog title="添加招生信息" :visible.sync="dialogFormVisible">
          <el-form :model="form">
            <el-form-item label="招生专业" :label-width="formLabelWidth">
        ...
        <el-dialog title="修改招生信息" :visible.sync="dialogFormVisible2">
          <el-form :model="form">
  ...
  export default {
    ...
    methods: {
      ...
      getmajordata:function(){
        let that=this;
        this.$http.get('/apprenticeship/major/getdata?page=1').then(function (res) {
          console.log(res.data)
          that.majorInfo=res.data.data;
        })
        this.major = true;
        this.classes = false;
        this.isActiveOne= true;
      ...
      insertmajor:function () {
        ...
        if (that.form.majorName!=''){
          this.$http.post('/apprenticeship/major/insertmajor', {
      ...
      deletemajor:function(majorId){
        this.$confirm('此操作将永久删除该专业, 是否继续?', '提示', {
          ...
          let that=this;
          this.$http.post('/apprenticeship/major/deletemajor',{majorId:majorId}).then(function (res) {
            console.log(res);
            if (res.data.message==1){
              alert("删除专业成功！");
      ...
      geteditmajor:function(majorId){
        ...
        let that = this;
        this.$http.post('/apprenticeship/major/geteditmajor',{majorId:that.majorId}).then(function (res) {
          console.log(res.data)
      ...
      updatemajor:function (majorId) {
        let that = this;
        if (that.form.majorName != ''){
          this.$http.post('/apprenticeship/major/altermajor', {
            majorId:majorId,
            majorName:that.form.majorName,
      ...
      querymajor:function(){
        this.$http.post('/apprenticeship/major/querymajor', {majorName: this.majorName}).then(res => {
