<template>
  <div class="container">
    <div class="qu-wrap">
      ...
      <div class="qu-content">
        <section class="qu-item"
          ...
          <template v-if="item.type === 'input'">
            <input style="margin-left: 40px" type="text" v-focus
                   v-model="answers[index]"
                   @focus="inputText = ''"
                   @blur="blurInput(answers[index],index)"
                   @keyup.esc="cancelOptionEdit()"
                   @keyup.enter="doneOptionEdit(index)">
          </template>
          <template v-if="item.type === 'rate'">
            <el-rate v-model="answers[index]" @change="blurInput(answers[index],index)"></el-rate>
          </template>
          <template v-if="item.type === 'textarea'">
            <textarea rows="8" cols="80"
                      v-model="answers[index]"
                      @blur="blurInput(answers[index],index)"
                      @keyup.enter="blurInput(answers[index],index)"></textarea>
          </template>
        ...
        <div class="add-box">
          <p class="qu-type" :class="{expand: isAdding}">
            <!--<span @click="addType('input')"><i class="iconplus-blanksfill" style="font-size: 16px;color: #00A5FE"></i>单项填空</span>-->
            <span @click="addType('radio')"><i class="icondanxuanti" style="font-size: 16px;color: #00A5FE"></i>多项填空</span>
            <!--<span @click="addType('checkbox')"><i class="iconduoxuanti" style="font-size: 16px;color: #00A5FE"></i>多选题</span>-->
            <span @click="addType('textarea')"><i class="iconwenben1" style="font-size: 16px;color: #00A5FE"></i>文本</span>
            <span @click="addType('rate')"><i class="iconxingxing" style="font-size: 16px;color: #00A5FE"></i>打分</span>
          </p>
      ...
      <footer>
        <div v-if="flag == 30" class="ctrl-part">
          <span @click="iterator = saveBtn(); iterator.next()">保存需求</span>
        </div>
      </footer>
    </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.questionTemplate = Data.template1;
      this.getData();
    },
    ...
    methods: {
      ...
      *saveBtn() {
        ...
        this.saveData();
        that.quData.type = 30;
        yield that.showPrompt(`确认要保存需求？`);
        if (that.quData.id == undefined){
          that.$http.post('/apprenticeship/require/insertsurvey', {
            quData:that.quData,
            username:that.user
          }).then((res)=>{
          ...
          that.$http.post('/requirement/require/editsurvey', {
            ...
            if(res.data.message==1){
              if (that.flag == 10){
                ...
                alert("量表修改成功！");
                that.$router.push({ path:'/practice/teacher'});
              }else{
                alert("问卷修改成功！");
