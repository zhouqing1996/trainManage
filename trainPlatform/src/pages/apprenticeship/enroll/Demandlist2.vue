<template>
  <div class="container">
    <el-button style="margin-left: 40px" type="warning" @click="$router.push({path:'/apprenticeship/enroll/demandedit',query:{id: 0,flag:30}})">+ 新建需求</el-button>
      ...
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          ...
          <span style="float: right;color: #df5000">
            <a v-if="item.status==4">未发布</a>
            <a v-if="item.status==5">已发布</a>
          </span>
          <span style="float: right;color: #df5000">需求状态:</span>
          <!--<el-button style="float: right; padding: 3px 0" type="text">操作按钮</el-button>-->
        </div>
        <div class="text item">
          <router-link :to="{ path: '/apprenticeship/enroll/demandview', query: { id: item.id,flag:10,status:item.status }}">
            <el-button plain size="mini"><i  class="iconchakan"></i>查看</el-button>
          </router-link>
          <el-button @click="iterator = editItem(item); iterator.next()" plain size="mini" v-show="item.status==4"><i  class="iconbianji1"></i>编辑</el-button>
          <el-button @click="iterator = publishItem(item,1); iterator.next()" plain size="mini" v-show="item.status==4"><i  class="iconfabu1" @click="iterator = publishItem(item,1); iterator.next()"></i>发布需求</el-button>
          <!--<router-link :to="{ path: '/requirement/surveypublish', query: { id: item.id }}" >-->
            <!--<el-button plain size="mini" v-show="item.status==4"><i  class="iconxitongguzhang"></i>发布管理</el-button>-->
          <!--</router-link>-->
          <el-button @click="iterator = publishItem(item,2); iterator.next()" plain size="mini" v-show="item.status==5"><i  class="iconfanhui"></i>取消发布</el-button>
          ...
          <!--<router-link :to="{ path: '/requirement/datastatistics', query: { id: item.id,flag:10 }}">-->
...
<script>
  ...
  export default {
    ...
    methods: {
      getdata:function(){
        let that=this;
        that.$http.post('/apprenticeship/require/getdata',{
          page:1,
          username:that.user
        }).then(function (res) {
      ...
      *editItem(item) {
        ...
        yield this.$router.push({path:'/apprenticeship/enroll/demandedit',query:{id: item.id,flag:30}});
      }
    }
