      ...
      <el-card class="box-card">
        ...
        <div class="text item">
          <router-link :to="{ path: '/apprenticeship/enroll/demandview', query: { id: item.id,flag:40,status:item.status }}">
            <el-button plain size="mini"><i  class="iconchakan"></i>填写</el-button>
          </router-link>
          ...
          <!--&lt;!&ndash;<el-button @click="iterator = temconversion(item.id);iterator.next()" plain size="mini"><i  class="icongongneng_fabuxuqiu"></i>转化模板</el-button>&ndash;&gt;-->
...
<script>
  ...
  export default {
    ...
    methods: {
      getdata:function(){
        let that=this;
        that.$http.post('/apprenticeship/require/getcomdata',{
          // that.$http.post('/apprenticeship/require/getcomdata',{
          page:1,
          username:that.user,
          flag:that.flag
