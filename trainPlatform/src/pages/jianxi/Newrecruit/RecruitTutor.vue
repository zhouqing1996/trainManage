...
<!--              见习的学生-->
              <el-form-item label="见习学生" v-if="props.row.status == 1">
    ...
    <el-tab-pane label="已结束" name="second">
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label="见习时间" v-show="props.row.status == 3">
      ...
      <div class="block" v-show="arrange == 1">
        <el-form ref="form" :model="form" label-position="left" inline>
          ...
          <el-form-item label="见习学生" class="b">
            <div v-if="student">
              <el-row v-if="student['thisApply']">
                <el-col :span="5">申请该见习的学生：</el-col>
                <el-col :span="19">
              ...
              <el-row v-if="student['arrange']">
                <el-col :span="5">安排其它见习的学生：</el-col>
                <el-col :span="19">
                  <el-checkbox-group v-model="form.student">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData () {
                let that = this
                this.$http.post('/probation/recruit/data', {power: this.$store.getters.showPower}).then(function (res) {
            ...
            dateEnsure () {
                ...
                let that = this
                this.$http.post('/probation/recruit/modifystatus', { id: this.row.id, status: 1, date: this.studentEnddate }).then(function (res) {
                    if (res.data.data == 1) {
                        that.getData()
            ...
            handleCancel (index, row) {
                let that = this
                this.$http.post('/probation/recruit/modifystatus', { id: row.id, status: 2 }).then(function (res) {
                    if (res.data.data == 1) {
                        that.getData()
                        common.$emit('change', '信息已修改！')
                    }
            ...
            edit (index, row) {
                if (row.status == 0) {
                    ...
                    let that = this
                    this.$http.post('/probation/recruit/gettstudent', { id: row.id }).then(function (res) {
            ...
            ensureArran () {
                let that = this
                this.$http.post('/probation/recruit/arrangest', { id: this.form.id, student: this.form.student }).then(function (res) {
                    that.arrange = 0
                    that.getData()
                    common.$emit('change', '信息已修改！')
                    console.log(res.data.data)
            ...
            getApplystu(data) {
                ...
                let that = this
                this.$http.post('/probation/recruit/getapplystu', { id: id }).then(function (res) {
        ...
        mounted () {
            this.getData()
            let that = this
            this.$http.post('/probation/recruit/getteacher').then(function (res) {
                console.log(res.data)
