            ...
            <el-table-column type="expand">
              <template slot-scope="props">
                <el-form label-position="left" inline class="demo-table-expand">
                  ...
                  <el-form-item label="见习学生" v-show="props.row.status == 1 || props.row.status == 3 || props.row.status == 4 || props.row.status == 5">
  ...
  import { common } from '../../../main'
    export default {
      ...
      methods: {
        ...
        getData () {
          let that = this
          this.$http.post('/probation/recruit/major').then(function (res) {
            that.major = res.data.data
            console.log(res.data.data)
          })
            this.$http.post('/probation/recruit/grade').then(function (res) {
                that.grade = res.data.data
                console.log(res.data.data)
            })
          this.$http.post('/probation/recruit/data', {power: this.power}).then(function (res) {
            console.log(res.data.data)
        ...
        editFunc1 (index) {
          ...
          this.nowrow = -1
          let that = this
          this.$http.post('/probation/recruit/modify', { row: row }).then(function (res) {
            that.getData()
        ...
        handleDelete (index, row) {
          if (row.status == 1 || row.status == 3 || row.status == 5) {
            ...
            let that = this
            this.$http.post('/probation/recruit/delete', { id: row.id }).then(function (res) {
              if (res.data.data == 1) {
                that.getData()
                common.$emit('change', '信息已修改！')
                that.$alert('删除成功！', '成功', {
        ...
        handleCancel (index, row) {
          if (row.status == 0) {
            ...
            let that = this
            this.$http.post('/probation/recruit/modstatus', {id: row.id, status: 4}).then(function (res) {
              that.getData()
              common.$emit('change', '信息已修改！')
              console.log(res.data.data)
            })
