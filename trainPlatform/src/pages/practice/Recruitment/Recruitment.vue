      ...
      methods: {
        ...
        getData () {
          ...
          this.$http.post('/practice/recruitment/major').then(function (res) {
            ...
            this.$http.post('/practice/recruitment/grade').then(function (res) {
                that.grade = res.data.data
                console.log(res.data.data)
            })
          this.$http.post('/practice/recruitment/data', {power: this.power}).then(function (res) {
            console.log(res.data.data)
        ...
        editFunc1 (index) {
          ...
          this.nowrow = -1
          let that = this
          this.$http.post('/practice/recruitment/modify', { row: row }).then(function (res) {
            that.getData()
        ...
        handleDelete (index, row) {
          if (row.status == 1 || row.status == 3 || row.status == 5) {
            ...
            let that = this
            this.$http.post('/practice/recruitment/delete', { id: row.id }).then(function (res) {
              if (res.data.data == 1) {
                that.getData()
                common.$emit('change', '信息已修改！')
                that.$alert('删除成功！', '成功', {
        ...
        handleCancel (index, row) {
          if (row.status == 0) {
            ...
            let that = this
            this.$http.post('/practice/recruitment/modstatus', {id: row.id, status: 4}).then(function (res) {
              that.getData()
              common.$emit('change', '信息已修改！')
              console.log(res.data.data)
            })
