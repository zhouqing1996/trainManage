...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      practiceSubmit () {
        console.log(this.practice,this.id)
        this.$http.post('/requirement/publish/submitpractice2', { content: this.practice.content, id: this.id, pushId: this.row.id }).then(function (res) {
          alert('系统推送成功！')
        })
      },
      practiceSet () {
        this.$http.post('/requirement/publish/setpractice2', { student: this.row.student, id: this.id }).then(function (res) {
          alert('设置成功！')
        })
      },
      visibleChange (val) {
