<template>
  <div class="container">
    <el-button style="margin-left: 40px" type="warning" @click="$router.push({path:'/requirement/surveyindex',query:{id: 0,flag:10}})">+ 新建问卷</el-button>
    <div v-for="(item,index) in surveyList" :key="index">
      <el-card class="box-card">
        ...
        <div class="text item">
          <router-link :to="{ path: '/requirement/surveyview', query: { id: item.id,flag:10,status:item.status }}">
  ...
  export default {
    ...
    methods: {
      ...
      *editItem(item) {
        ...
        yield this.$router.push({path:'/requirement/surveyedit',query:{id: item.id,flag:10}});
      }
    }
